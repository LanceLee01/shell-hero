...
export class TalentScene extends Phaser.Scene {
  private save!: ReturnType<typeof loadSave>
  ...
  private getSlot(): SaveSlot {
    return getActiveSlot(this.save)
  }

  private updateCrystalText() {
    const slot = this.getSlot()
    this.crystalText.setText("晶石: " + slot.crystals)
  }
  ...
  private renderTalents() {
    ...
    TALENTS.forEach((t, i) => {
      ...
      const canAfford = slot.crystals >= cost && !maxed
      ...
      if (maxed) {
        ...
        zone.on("pointerdown", () => {
          slot.talents[t.id] = (slot.talents[t.id] || 0) + 1
          slot.crystals -= cost
          slot.lastSave = new Date().toISOString()
          saveSave(this.save)
          this.updateCrystalText()
          this.renderTalents()
        })
